...
class VariantProfile:
    ...
    # Inner stream: N Mamba-only think passes before the main forward (Option B state-anchored).
    # Disabled by default; enable in V3.8 variants to activate the think token pipeline.
    enable_think_tokens: bool = False
...
class LifeEquationConfig:
    ...
    # ── V3.8 additions ──────────────────────────────────────────────────────────
    # MIMO: independent B,C projections per SSM head (ngroups = n_mamba_heads = d_model/d_state).
    enable_mimo: bool = True
    ...
    alpha_sal: float = 3.0
    # Inner stream think tokens (Option B state-anchored, K=16).
    n_think_tokens: int = 16        # N think-token passes before each main forward
    think_anchor_every: int = 16    # Inject state_proj(Z_cog[-1]) anchor every K steps
