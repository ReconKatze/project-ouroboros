...
class LifeEquationModel(nn.Module):
    def __init__(
        ...
        # V3.8: MIMO + T_identity differentiation per block.
        # First n_identity_mamba blocks: slow decay (A_log→0) + low rope_fraction = identity subspace.
        # Remaining blocks: standard 0.5 rope_fraction, MIMO ngroups = n_mamba_heads.
        self.mamba_layers = nn.ModuleList([
            Mamba3Block(
                self.config,
                layer_idx=i,
                is_mimo=self.config.enable_mimo,
                rope_fraction=(
                    self.config.identity_rope_fraction
                    if i < self.config.n_identity_mamba
                    else 0.5
                ),
                is_identity_block=(i < self.config.n_identity_mamba),
            )
            for i in range(self.config.n_mamba_layers)
        ])
        ...
        # V3.8: Inner stream think module (Option B state-anchored).
        # Always instantiated (negligible param cost: d_model + d_model²).
        # Active only when profile.enable_think_tokens=True.
        self.think_module = ThinkStreamModule(self.config)
    ...
    def _controller_step(
        ...
        u_t = self.controller.build_input(
            state.Z_eps, d_id, c_cont, s_compat, boredom, z_ovr, coherence, v_self, gamma_eff,
            z_learn=state.Z_learn,   # V3.8: learning rate signal for controller
        )
    ...
    def forward(
        ...
        # V3.8 Inner stream: N Mamba-only think passes enrich Z_ssm and Z_cog before
        # the main forward.  Skipped during warmup (weights still random) and consolidation.
        if self.profile.enable_think_tokens and warmup >= 1.0 and not consolidating:
            state = self.think_module(self.mamba_layers, state)
        ...
        for layer_index in range(self.config.n_layers_total):
            ...
            # V3.8 Sal_surprise: sigmoid(alpha_sal * ||error|| / sqrt(d_model)).
            # ≈ 0.5 when error ≈ 0 (well-predicted), rises toward 1.0 for surprising tokens.
            # Multiplies the gated error so large surprises receive more Mamba bandwidth.
            sal_weight = torch.sigmoid(
                self.config.alpha_sal
                * error_seq.norm(dim=-1, keepdim=True)
                / math.sqrt(self.config.d_model)
            )  # [B, seq_len, 1]
            gated_error_seq = sal_weight * (att_gain_b * error_seq - friction_b)
        ...
        # V3.8: INSPECT_MEMORY — deepen episodic read using post-Mamba layer_input as query.
        # layer_input carries richer semantic content than the raw x_t used in the Phase C read.
        elif fire and action == "INSPECT_MEMORY" and self.profile.enable_memory_read:
            inspect_read = self.epi_module.read(layer_input, state, warmup)
            layer_input = layer_input + inspect_read
        ...
        # Entropy regularization: subtract entropy to maximize action distribution entropy.
        ...
        # Lowered from 0.03 → 0.005 at step 4000: sampling (multinomial) already prevents argmax
        # collapse, so λ=0.03 over-regularised — pushed entropy to ~ln(4) and the policy could
        # not develop any contextual preferences (recall stagnated at 0.728→0.734 over 1000 steps,
        # action sequence looked like random thrashing every 1-3 steps).  0.005 allows sharpening.
        losses["L_ctrl"] = l_supervised_policy + (utility - l_actual_improvement).pow(2).mean() - 0.005 * ctrl_entropy
