...
class Mamba3Block(nn.Module):
    ...
    def __init__(
        ...
        # MIMO: ngroups = n_mamba_heads gives each head its own B,C projections.
        # ngroups=1 (default) shares B,C across all heads (less expressive, fewer params).
        n_mamba_heads = config.d_model // config.d_state
        ngroups = n_mamba_heads if is_mimo else 1
        # Pass layer_idx so the inference_params path can use it as the state dict key.
        # Try full V3.8 signature first, then fall back for older mamba-ssm builds.
        constructed = False
        for kwargs in [
            dict(d_model=config.d_model, d_state=config.d_state, headdim=headdim,
                 rope_fraction=rope_fraction, ngroups=ngroups, layer_idx=layer_idx),
            dict(d_model=config.d_model, d_state=config.d_state, headdim=headdim,
                 rope_fraction=rope_fraction, ngroups=ngroups),
            dict(d_model=config.d_model, d_state=config.d_state, headdim=headdim,
                 rope_fraction=rope_fraction, layer_idx=layer_idx),
            dict(d_model=config.d_model, d_state=config.d_state, headdim=headdim,
                 rope_fraction=rope_fraction),
        ]:
            try:
                self.mamba = Mamba3(**kwargs)
                constructed = True
                break
            except TypeError:
                continue
        if not constructed:
            raise RuntimeError("Could not construct Mamba3 — no compatible signature found.")

        if is_identity_block:
            self._init_identity_decay()
    ...
    def _init_identity_decay(self) -> None:
        """T_identity: set A_log → 0.0 so SSM decay ≈ 1 (near-identity recurrence).

        The first n_identity_mamba blocks start with minimal decay so their recurrent
        state tracks the input almost perfectly, creating a stable identity-anchored
        subspace.  Training will deviate from this init, but the inductive bias persists.
        """
...
class ControllerModule(nn.Module):
    ...
    def __init__(self, config: LifeEquationConfig):
        ...
        # 9-dim input: eps_norm, gamma_eff*D_id, 1-C_cont, s_compat, boredom,
        #              homeo_ovr_norm, coherence, V_self, z_learn_norm
        self.policy = nn.Linear(9, len(self.ACTIONS))
    ...
    def build_input(
        ...
        z_learn: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        ...
        # z_learn_norm: EMA of distill_loss — signals how well the model is currently learning.
        # Near zero = fast learner; high = struggling. Informs controller whether to persist.
        z_learn_norm = (
            z_learn.norm(dim=-1, keepdim=True)
            if z_learn is not None
            else torch.zeros_like(coherence)
        )
        return torch.cat(
            [
                ...
                z_learn_norm,   # v38: learning rate signal
            ],
            dim=-1,
        )
...
class BlockAttnResidual(nn.Module):
    ...
    -----------
    n_anchors : int
        Number of attention anchors (4 for life_eq_v38).  We allocate n_anchors
...
class ThinkStreamModule(nn.Module):
    """Option B state-anchored inner stream (inner_stream.md).

    Runs N Mamba-only think passes before the main forward.  Every anchor_every steps
    injects a learned projection of Z_cog[-1] to prevent the think stream from drifting
    away from actual state (K=16 grounding, as per the spec).

    Updates state.Z_ssm and state.Z_cog so the main forward inherits richer context.
    The think pass is differentiable — gradients flow through mamba_layers as normal.
    Only the Z_cog slots are detached (matching the same convention used in the main loop).

    Skipped during warmup and consolidation passes (caller's responsibility).
    """

    def __init__(self, config: LifeEquationConfig):
        super().__init__()
        self.config = config
        # Learned "thinking" token embedding — zero init so early training is stable.
        self.think_embed = nn.Parameter(torch.zeros(config.d_model))
        # Anchor projection: Z_cog last-layer → grounding token every K think steps.
        # Zero-init: anchor contributes nothing at start, grows via gradient.
    ...
    def forward(
        ...
        state: "FullState",  # type: ignore[name-defined]
    ) -> "FullState":  # type: ignore[name-defined]
        """Run N think tokens through all Mamba layers, updating Z_ssm and Z_cog.

        Args:
            mamba_layers : nn.ModuleList of Mamba3Block (the model's shared blocks)
            state        : FullState — mutated in place (Z_ssm, Z_cog)
        Returns:
            state (mutated)
        """
        n_think = self.config.n_think_tokens
        anchor_every = self.config.think_anchor_every
        batch = state.Z_cog.shape[0]
        device = state.Z_cog.device
        n_mamba = len(mamba_layers)

        current_ssm: List[Optional[torch.Tensor]] = (
            state.Z_ssm if state.Z_ssm is not None else [None] * n_mamba
        )
        z_cog_last = state.Z_cog[:, -1, :]   # [B, d_model] — last layer's cognitive state

        # Build think sequence [B, n_think, d_model] with periodic state anchors.
        tokens: List[torch.Tensor] = []
        for i in range(n_think):
            if i > 0 and i % anchor_every == 0:
                # State-anchored: project last Z_cog layer to ground the think stream.
                anchor = self.anchor_proj(z_cog_last)   # [B, d_model]
                tokens.append(anchor.unsqueeze(1))
            else:
                tokens.append(self.think_embed.view(1, 1, -1).expand(batch, 1, -1))
        think_seq = torch.cat(tokens, dim=1)   # [B, n_think, d_model]

        # Run think sequence through all Mamba layers in order, threading SSM state.
        new_cog_slots: List[torch.Tensor] = []
        new_ssm: List[Optional[torch.Tensor]] = []
        seq = think_seq
        for m_idx, mamba_block in enumerate(mamba_layers):
            think_out = mamba_block(seq, initial_states=current_ssm[m_idx])
            cap = mamba_block._last_final_states
            new_ssm.append(cap if isinstance(cap, torch.Tensor) else None)
            cog_slot = think_out[:, -1, :]
            _cn = cog_slot.norm(dim=-1, keepdim=True)
            cog_slot = cog_slot / _cn.clamp(min=float(self.config.d_model) ** 0.5)
            new_cog_slots.append(cog_slot.detach())
            seq = think_out   # each layer sees the previous layer's think output

        state.Z_ssm = new_ssm
        state.Z_cog = torch.stack(new_cog_slots, dim=1)   # [B, n_mamba, d_model]
        return state
