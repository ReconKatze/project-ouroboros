...
VARIANT_PROFILES: Dict[str, VariantProfile] = {
    ...
    # ── V3.8 variants ────────────────────────────────────────────────────────────
    "v38_flagship": _profile(
        "v38_flagship",
        "Full V3.8 flagship variant: MIMO head specialisation (ngroups=n_heads), "
        "T_identity slow-decay blocks (first 8 Mamba blocks, A_log→0, rope_fraction=0.1), "
        "Sal_surprise intrinsic curiosity multiplier on Mamba gate, Z_learn in controller "
        "input (9-dim policy), INSPECT_MEMORY fully wired, and Inner Stream think tokens "
        "(N=16 Mamba-only passes before main forward, Option B state-anchored K=16). "
        "All modules active: narrative, sleep/dream, trust, Block AttnRes, ACT looped "
        "attention, SelfDynamicsModel. theta_vol=-1.0 suppresses early VOLUNTARY_END.",
        training_focus=(
            "think_stream", "mimo", "t_identity", "sal_surprise",
            "controller", "looped_attention", "adaptive_halting",
            "narrative_coherence", "trust_dynamics", "self_dynamics",
        ),
        controller_mode="live",
        enable_narrative=True,
        enable_sleep_dream=True,
        ...
        enable_attn_halting=True,
        enable_self_dynamics=True,
        enable_think_tokens=True,
        theta_vol=-1.0,
    ),
    "v38_no_think": _profile(
        "v38_no_think",
        "V3.8 flagship without think tokens — ablation to isolate think-stream contribution. "
        "All other V3.8 features active (MIMO, T_identity, Sal_surprise, Z_learn, INSPECT_MEMORY).",
        training_focus=(
            "mimo", "t_identity", "sal_surprise", "controller",
            "looped_attention", "adaptive_halting", "narrative_coherence",
            "trust_dynamics", "self_dynamics",
        ),
        controller_mode="live",
        enable_narrative=True,
        enable_sleep_dream=True,
